define(
    [
        ...
        'Shared/I18n',
        'Shared/Utility',
        'Flownative.NeosSortableEditor/Library/Sortable'
    ],
                                ...
                                $(result.resource).find('li').each(function (index, value) {
                                    ...
                                    data.results.push({
                                        ...
                                        data: {
                                            ...
                                            path: Utility.removeContextPath($('.node-frontend-uri', this).text().trim().replace('.html', '')),
                                            nodeType: $('.node-type', value).text()
                                        }
                                    });
                                });
                                query.callback(data);
                ...
                if (value && value !== currentValue) {
                    ...
                    $(JSON.parse(value)).each(function (index, nodeIdentifier) {
                        ...
                        HttpRestClient.getResource('neos-service-nodes', nodeIdentifier, {data: parameters}).then(function (result) {
                            ...
                            item.set('data', {
                                identifier: $('.node-identifier', result.resource).text(),
                                path: Utility.removeContextPath($('.node-frontend-uri', result.resource).text().trim().replace('.html', '')),
                                nodeType: $('.node-type', result.resource).text()
                            });
                            that._updateSelect2();
                        });
